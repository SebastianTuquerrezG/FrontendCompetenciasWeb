...
export const createRA = async (ra: { descripcion: string; idCompetenciaPrograma: number; estado: 1 }) => {
  try {
    console.log(ra);
...
export const updateRA = async (ra: { id: number, descripcion: string; idCompetenciaPrograma:  number; estado: number }) => {
  try {
    console.log(ra);
