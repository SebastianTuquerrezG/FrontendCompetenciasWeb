...
import { getRubricas, updateRubricas } from "@/services/Rubrica/rubricas.service";
import { getRAAsig } from "@/services/Competencias_Ra_service/raAsig.service";

const InactivasRubricas = () => {
    ...
    const handleSave = async (id: number, raAsignaturaId: number, createAt: Date, updateAt: Date) => {
        try {
            const updatedRubrica = await updateRubricas({ id, nombre: editDescription, raAsignaturaId: raAsignaturaId, estado: "INACTIVO", createAt: createAt, updateAt: updateAt}) as { id: number; nombre: string; raAsignaturaId: number; estado: string;  createAt: Date, updateAt: Date };
    ...
    const handleActivate = async (record: { id: number; nombre: string; raAsignaturaId: number; estado: string;  createAt: Date, updateAt: Date;}) => {
        try {
            let id = record.id;
            let descripcion = record.nombre;
            let raAsignatura = record.raAsignaturaId;
            
            if (id !== null) {
                const updatedRubrica = await updateRubricas({ id, nombre: descripcion, raAsignaturaId: raAsignatura, estado: "ACTIVO", createAt: record.createAt, updateAt: record.updateAt}) as { id: number; nombre: string; raAsignaturaId: number; estado: string;  createAt: Date, updateAt: Date };
                setRecords(records.map(record => (record.id === id ? updatedRubrica : record)));
            }
        } catch (error) {
...
export default InactivasRubricas;